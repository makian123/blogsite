...
/// # Example
/// ```
/// let database_connection: Arc<Pool<ConnectionManager<PgConnection>>> = psql_connect_to_db();
/// ```
pub fn psql_connect_to_db(max_size: Option<u32>) -> Arc<Pool<ConnectionManager<PgConnection>>> {
    dotenv().ok();

    let database_url = env::var("DATABASE_URL").expect("Enviroment variable: 'DATABASE_URL' not set");

    let psql_manager = ConnectionManager::<diesel::PgConnection>::new(database_url);
    match max_size{
        Some(size) => {
            Arc::new(Pool::builder().max_size(size).build(psql_manager).unwrap())
        },
        None => {
            Arc::new(Pool::new(psql_manager).unwrap())
        },
    }
}
...
/// # Example
/// ```
/// let database_connection: Arc<Pool<RedisConnectionManager>> = redis_connect_to_db();
/// ```
pub fn redis_connect_to_db(max_size: Option<u32>) -> Arc<Pool<RedisConnectionManager>> {
    ...
    match max_size{
        Some(size) => {
            Arc::new(Pool::builder().max_size(size).build(redis_manager).unwrap())
        },
        None => {
            Arc::new(Pool::new(redis_manager).unwrap())
        },
    }
}